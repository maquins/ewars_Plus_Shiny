...
Out.Mod<<-eventReactive(c(
  ...
  input$new_model_Year_validation),
  {
    ...
    alarm_indicators<-var_names_New_model()$vars_in_Model
    cat(paste('\nAlarm indicators now ..\n'),paste(var_names_New_model()$vars_in_Model,collapse =','),'\n\n')
    ...
    nlag <<- var_names_New_model()$nlags
    #nlag <<- 12
    alarm_vars<<-var_names_New_model()$vars_in_Model
    ...
    Model_pred_Eval<-readRDS("Model_pred_Eval.rds")
    ...
    pred_vals_all_promise<<-future_promise({
      ...
      #list_Save1<-list(pred_vals_all=pred_Eval,
                       #res_promise=res_m)
      #saveRDS(list_Save1,"~/Library/CloudStorage/GoogleDrive-ewarsplusdemo@gmail.com/My Drive/ewars_Plus_demo_Files/Model_pred_Eval.rds",
              #compress =T)
      #foreach(a=1:nrow(run_grid),.combine =rbind)%do% get_weekly_prediction(a)
      #readRDS("pred_eval_2011_2013.rds")
      #db_fl<-readRDS("~/Library/CloudStorage/GoogleDrive-ewarsplusdemo@gmail.com/My Drive/ewars_Plus_demo_Files/ewars_Plus_DB_files.rds")
      Model_pred_Eval$pred_vals_all %>% 
        data.frame() %>% 
        dplyr::select(-index) %>% 
        dplyr::filter(year %in%  Years_orig)
    },
    ...
    list(pred_vals_all_promise=pred_vals_all_promise,
         ...
         nlags=nlag,
         all_endemic=var_names_New_model()$all_endemic)
  },
  ignoreNULL=F)
